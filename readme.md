...
Users can manually provide values in the user interface for:
...
Users can upload a CSV containing the following columns:
...
The application returns a downloadable CSV containing corresponding model predictions.
...
- Model insight through feature importance  
- Detailed feature explanations  
- Automated preprocessing, including:
  - One-hot encoding  
  - Standard scaling  
  - PowerTransform  
  - Wind direction conversion using sine and cosine  

## Troubleshooting

If dependency issues occur, manually install:
...
If model files are missing, verify that all `.joblib` and `.cbm` files are present in the application directory.
...
This application demonstrates a complete ML deployment pipeline, from preprocessing and model training to production-ready inference.
```
